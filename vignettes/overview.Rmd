...
the only required data is soil properties data and a given simulation period.
...
* Crop C inputs (B_C_OF_INPUT, kg C/ha). If this is not known, it can be calculated with known crop harvest characteristics using `rc_calculate_b_c_of`.
...
* Mean temperature (W_TEMP_MONTH_MEAN, °C)
* Mean precipitation (W_PREC_MONTH_SUM, mm)
* Actual evapotranspiration (W_ET_ACT_MONTH, mmm)
                  OR
* Reference evapotranspiration (W_ET_REF_MONTH, mm)
* factor to recalculate reference to actual evapotranspiration (W_ET_REFACT, unitless). Defaults to 0.75 when not provided.
...
**Note: initialization calculations are not yet implemented. The below will be active once ghsolver branch is merged**
...
If C stocks are in equilibrium, initialize should be set using 'spinup_analytical_heuvelink'.
...
data to represent the different fields/scenaro's/ This example shows multicore calculations with similar
...
soil <- data.table(
  ...
  # plot result
  ggplot(data = result)+
  geom_line(aes(x = time, y =A_SOM_LOI, linetype = ID))+
  labs(x = "year", y = "OM [%]") +
    theme_bw()
