...
RothC is well-established model to calculate the development of carbon (C) stocks in soils
...
inputs from crops and amendments, the model changes in C stocks in a monthly resolution.
...
An important aspect of RothC is the initialization of the C pools. Rotsee implements using
either pre-defined pools, or using initialization calculations.
...
SImulation duration can be directly set using start_date and end_date parameters.
In case these parameters are not provided, the simulation period is based on earliest 

## 5. model output
Model calculation provides C stock of the soil as a function of time. Depending on 
...
For this we refer you to the vignette \link{helpers.Rmd}.
