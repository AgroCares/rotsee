...
test_that("rc_input_event_crop validates input parameters correctly", {
  ...
  crops_missing_cols <- data.table::data.table(
    B_LU = "nl_1",
    year = 2020
  )
  expect_error(
  ...
  crops_invalid_yield <- data.table::data.table(
    B_LU = "nl_1",
    year = 2020,
  ...
  crops_invalid_timing <- data.table::data.table(
    B_LU = "nl_1",
    year = 2020,
  ...
  crops_negative_yield <- data.table::data.table(
    B_LU = "nl_1",
    year = 2020,
    M_GREEN_TIMING = "august",
    M_CROPRESIDUE = TRUE,
...
test_that("rc_input_event_crop handles valid inputs correctly", {
  ...
  crops_valid <- data.table::data.table(
    B_LU = c("nl_1", "nl_2"),
    year = c(2020, 2021),
...
test_that("rc_input_event_crop handles catch crop timing correctly", {
  ...
  for (timing in timing_values) {
    crops_test <- data.table::data.table(
      B_LU = "nl_1",
      year = 2020,
...
test_that("rc_input_event_crop handles mandatory catch crops on sandy soils", {
  ...
  crops_maize_sandy <- data.table::data.table(
    B_LU = "nl_1", # Assuming this maps to maize in rc_crops
    year = 2020,
...
test_that("rc_input_event_crop handles crop residue management correctly", {
  ...
  crops_with_residue <- data.table::data.table(
    B_LU = "nl_1",
    year = 2020,
    M_GREEN_TIMING = "august",
    M_CROPRESIDUE = TRUE,
  ...
  crops_without_residue <- data.table::data.table(
    B_LU = "nl_1",
    year = 2020,
    M_GREEN_TIMING = "august",
    M_CROPRESIDUE = FALSE,
...
test_that("rc_input_event_crop handles edge cases for cf_yield", {
  ...
  crops_min_yield <- data.table::data.table(
    B_LU = "nl_1",
    year = 2020,
    M_GREEN_TIMING = "august",
    M_CROPRESIDUE = TRUE,
  ...
  crops_max_yield <- data.table::data.table(
    B_LU = "nl_1",
    year = 2020,
    M_GREEN_TIMING = "august",
    M_CROPRESIDUE = TRUE,
...
test_that("rc_input_event_crop handles multiple years correctly", {
  ...
  crops_multi_year <- data.table::data.table(
    B_LU = rep(c("nl_1", "nl_2"), 3),
...
test_that("rc_input_event_crop handles empty input gracefully", {
  ...
  crops_empty <- data.table::data.table(
    B_LU = character(0),
    year = integer(0),
...
test_that("rc_input_event_crop handles different crop types", {
  ...
  crop_types <- c("nl_1", "nl_20", "nl_100", "non_nl_crop")
